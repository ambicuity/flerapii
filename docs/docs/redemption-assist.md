...
- **Intelligent Account Matching**: Based on the current page URL, it automatically finds the most matching site account from Flerapii, eliminating the need to manually search through lists.
...
1. **Corresponding Site Account Exists in Flerapii**:
...
1. Confirm that "Redemption Assist" is enabled in Flerapii settings;
