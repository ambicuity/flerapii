...
Flerapii treats “keys” as upstream API tokens managed by supported backends (One-API/New-API family, OneHub, etc.) via `GET/POST /api/token/…` (see `services/apiService/common/*` and per-site overrides).
