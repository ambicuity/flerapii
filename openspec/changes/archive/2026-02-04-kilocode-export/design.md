...
Issue #364 requests a way to export Flerapii account/token configuration into **Kilo Code** (and Roo Code) settings JSON so users with many sites can avoid manual re-entry in IDE assistants.
...
**Rationale:** This matches Kilo Code/Roo Code “OpenAI Compatible” configuration semantics, and is compatible with the way Flerapii sites are used (OpenAI-style relay endpoints).
