...
Some users only receive an API relay credential bundle (base URL + API key) and do **not** have an admin/dashboard account to add a site account inside Flerapii. Today those users can run ad-hoc checks via Web AI API Check, but they cannot **persist** and **reuse** these credentials across the extension (verification, exports, copy actions).
