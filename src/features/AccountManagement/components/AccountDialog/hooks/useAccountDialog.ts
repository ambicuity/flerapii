...
export function useAccountDialog({
  ...
  const handleSaveAccount = async (): Promise<AccountSaveResponse | null> => {
    try {
      setIsSaving(true)
      ...
      if (!result.success) {
        toast.error(result.message || t("messages.saveFailed"))
        return null
      }
      ...
      logger.error("Account save failed", {
        error,
        mode,
  ...
  const handleAutoConfig = async () => {
    ...
    try {
      ...
      // account
      if (!targetAccount) {
        const savedAccount = await handleSaveAccount()
        targetAccount = savedAccount?.accountId
        if (!targetAccount) {
          return
        }
        //  ref
  ...
  const handleUrlChange = (newUrl: string) => {
    duplicateAccountWarningAcknowledgedSiteUrlRef.current = null
    if (newUrl.trim()) {
      const origin = tryParseOrigin(newUrl)
