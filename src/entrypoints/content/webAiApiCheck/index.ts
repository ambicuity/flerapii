...
import {
  ...
  isRuntimeMessageConnectionError,
  sendRuntimeMessage,
...
function setupWebAiApiCheckDetection() {
  ...
  let lastScanAt = 0
  let runtimeEndpointUnavailable = false
  let hasLoggedRuntimeEndpointUnavailable = false

  const handleRuntimeEndpointUnavailable = (error: unknown): boolean => {
    if (!isRuntimeMessageConnectionError(error)) {
      return false
    }

    runtimeEndpointUnavailable = true

    if (!hasLoggedRuntimeEndpointUnavailable) {
      hasLoggedRuntimeEndpointUnavailable = true
      logger.debug(
        "ApiCheck runtime endpoint unavailable; auto-detect is disabled until the page reloads",
        error,
      )
    }
  ...
  const requestShouldPrompt = async (pageUrl: string): Promise<boolean> => {
    ...
    try {
      ...
      if (!handleRuntimeEndpointUnavailable(error)) {
  ...
  const handleClipboardEvent = (event: ClipboardEvent) => {
    ...
    const pageUrl = window.location.href
    if (!/^https?:/i.test(pageUrl)) return
    if (runtimeEndpointUnavailable) return
