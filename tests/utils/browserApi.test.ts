...
describe("browserApi isRuntimeMessageConnectionError", () => {
  it("matches stale or missing runtime endpoint errors", () => {
    expect(
      isRuntimeMessageConnectionError(
        new Error(
  ...
  it("ignores unrelated failures", () => {
    expect(
      isRuntimeMessageConnectionError(new Error("Permission denied")),
    ).toBe(false)
    expect(isRuntimeMessageConnectionError({})).toBe(false)
  })
})
