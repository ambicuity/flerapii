...
describe("anyrouterProvider", () => {
  ...
  describe("checkIn", () => {
    it("returns success on successful check-in (message includes Check in successful)", async () => {
      ...
      mockedFetchApi.mockResolvedValueOnce({
        ...
        ret: 1,
        success: true,
        message: "Check in successful, got $25",
      })
    ...
    it("returns already_checked when request throws and error message indicates already checked", async () => {
      ...
      mockedFetchApi.mockRejectedValueOnce(new Error("already checked in"))
